...
function renderSearchResult(result){
	tests = [];
	unblockTree();
	createPaginationBlock(document.getElementById("divPagination"),result.numberOfResults, result.page, result.limit, "onPageClick");
    ...
    for(var i=0; i<result.testProjects.length;i++) {
	    ...
	    html+="<input id='chkTestProject_"+i+"' type='checkbox' onchange='changeSelections("+projectTestIds+",this.checked);'/>";
	    ...
	    for(var j=0; j<project.testGroups.length; j++) {
            ...
            html+="<input id='chkTestGroup_"+j+"' type='checkbox' onchange='changeSelections("+testGroupTestIds+",this.checked);'/>";
            html+="<img src='../images/workflow-icon-test-group.png'/> ";
            if(testGroup.name!=null) {
                   html+=escapeHTML(testGroup.name);                           
               }
               else html+="<span style='color:gray;'>Unknown Group</span>";
            ...
            for(var k=0; k<testGroup.tests.length; k++) {
	    		...
	    		html+="<li>";
                html+="<div onMouseDown='onTestInTreeMouseDown("+testGroup.tests[k].id+",this); return false;'>";
                html+="<input id='chkTest_"+testGroup.tests[k].id+"' type='checkbox' onchange='changeSelection("+testGroup.tests[k].id+",this.checked);'/>";
                ...
                html+="<a href='../test/display?id="+testGroup.tests[k].id+"' target='_blank'>";
                html+="<img src='../images/workflow-icon-test.png'/>";
                html+=escapeHTML(testGroup.tests[k].name);
                html+="</a>";
                html+="</div>";
                html+="<div id='testTooltip"+testGroup.tests[k].id+"' style='display:none;' class='tooltip'><pre>"+escapeHTML(testGroup.tests[k].description)+"</pre></div>";
                html+="</li>";
            }
...
function showTestDetails(testId){
	...
	if(desc==null)desc="";
	str+="<b>"+test.name+"</b><br/><br/>"+desc.replace(/\n/g,'<br />');
	str+="<br/><br/>";
	div.innerHTML = str;
...
<div id="treeNavigationPanel" style="height: 300px; width: 240px;overflow:auto;background:white;border:1px solid #aaa;padding:4px;margin:0px;">
    ...
    <div id="divTestList" class="test-search-list" style="width: 100%; height: 100%;"></div>
</div>
<script>
